...
def execute_scikit_kmeans(fileName):
    """Example"""
...
def execute_scikit_kmeans_with_PCA(fileName):
    np.set_printoptions(formatter={'float':lambda x: '%.2f'%x})
...
def execute_scikit_kmeans_KMeans_plus(fileName):
    np.set_printoptions(formatter={'float':lambda x: '%.2f'%x})
    [sampleData,groundTruth]=prepare_data(fileName)
    kMeansOut=KMeans(n_clusters=2,init='k-means++',max_iter=25).fit(sampleData)
    print_final_results(kMeansOut.labels_, kMeansOut.cluster_centers_, groundTruth)

def execute_scikit_kmeans_KMeans_random(fileName):
    np.set_printoptions(formatter={'float':lambda x: '%.2f'%x})
    [sampleData,groundTruth]=prepare_data(fileName)
    kMeansOut=KMeans(n_clusters=2,init='random',max_iter=25).fit(sampleData)
    print_final_results(kMeansOut.labels_, kMeansOut.cluster_centers_, groundTruth)

def execute_scikit_kmeans_MiniBatch_KMeans(fileName):
    np.set_printoptions(formatter={'float':lambda x: '%.2f'%x})
    [sampleData,groundTruth]=prepare_data(fileName)
...
execute_scikit_kmeans("small.csv")
execute_scikit_kmeans_with_PCA("pcaAttributes.csv")
execute_scikit_kmeans("car.csv")
execute_scikit_kmeans_with_PCA("pca_attributes_car.csv")
execute_scikit_kmeans_KMeans_plus("car.csv")
execute_scikit_kmeans_KMeans_random("car.csv")
execute_scikit_kmeans_MiniBatch_KMeans("car.csv")